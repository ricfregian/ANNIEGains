...
with open("/Users/giancaceresvera/Desktop/ANNIE/calibration/gains_json_files/LEDthr_gains/LEDthr_gains_Gauss1_fitrng_5runs.json","r") as f:
    dat = json.load(f)
pdf = pd.DataFrame(dat["Gauss1"])
dat["Gauss1"].keys()

with open("/Users/giancaceresvera/Desktop/ANNIE/calibration/gains_json_files/LEDmaxpeak_gains/LEDmaxpeak_2runs_combined.json","r") as f:
    dat = json.load(f)
...
fig4,ax4 = plt.subplots()
#TUBES = np.arange(332,464,1)
TUBES = np.arange(352,372,1) #ETEL
for cnum in TUBES:
	if ((cnum not in list(pdf["Channel"])) or (cnum not in list(pdf1["Channel"]))):
            print("DID NOT FIND CHANNELNUM %i IN BOTH DATASETS"%(cnum) )
            continue
        	...
        	if(cnum > 351 and cnum < 372):
        	 ax4.errorbar(cnum,myy/myx, alpha = 1.0, yerr = yyerr ,label="%i Data"%cnum,linestyle='None',marker='D',markersize=5, markerfacecolor ="green", markeredgecolor ="green", ecolor="green")
       		 print("PMT %i, threshold/max.peak  = %f " %(cnum,myy/myx))
...
plt.axline((320.0, 1.0), slope=0.0, color="black", linestyle=(0, (5, 5)))
plt.xlabel("Channel")
plt.ylabel("AmBe threshold/LED max. peak")
#plt.ylabel("AmBe source/Gains DB")
#plt.ylabel("LED/Gains DB")
plt.ylim([0.7,2.5])
plt.title("")
